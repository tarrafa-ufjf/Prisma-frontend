...
The Local Database of Prisma uses **PostgreSQL** and is responsible for storing the configurations, processing status, and indicators calculated by the Analysis System.

Its main purpose is to persist processed results, allowing the API and Front-End to retrieve indicators without having to execute the entire analysis process again for each request.

The database is organized into tables that store information at different levels of granularity, including:

- **System configuration and control**;
- **Subject processing status**;
- **Individual indicator processing status**;
- **Student indicators**;
- **Student indicators aggregated by subject**;
- **Tutor indicators**;
- **Tutor indicators aggregated by subject**;
- **Scheduler task status**.

### Database structure

The main tables of the Local Database are:

| Table | Level | Purpose |
| --- | --- | --- |
| `configs` | Institution | Stores the connection configuration to the institutional database and the Moodle version in use. |
| `subjects_status` | Subject | Stores the processing status of a subject. |
| `subject_indicator_status` | Subject / Indicator / Actor | Controls the processing status of each indicator for a subject and actor type. |
| `local_indicators_students` | Student / Subject | Stores the metrics and classifications of each student's indicators. |
| `global_indicators_students` | Subject | Stores student indicators aggregated at the subject level. |
| `local_indicators_tutors` | Tutor / Subject | Stores the metrics and classifications of each tutor's indicators. |
| `global_indicators_tutors` | Subject | Stores tutor indicators aggregated at the subject level. |
| `scheduler_status` | System | Stores the status and execution information of scheduled processes. |

### `configs` table

The `configs` table stores the information required to establish the Prisma connection to the Institutional Database.

| Column | Type | Description |
| --- | --- | --- |
| `institution_id` | `Integer` | Institution identifier. |
| `version` | `String(40)` | Moodle version used by the institution. |
| `host` | `String` | Institutional database server address. |
| `port` | `Integer` | Port used for the connection. |
| `database` | `String` | Name of the institutional database. |
| `user` | `String` | Username used for the connection. |
| `password` | `String(512)` | Connection password, stored in encrypted form. |
...
The password stored in this table is encrypted using the `encrypt_config_secret()` and `decrypt_config_secret()` functions.

### `subjects_status` table

The `subjects_status` table controls the processing status of subjects.

| Column           | Type         | Description                              |
| ---------------- | ------------ | ---------------------------------------- |
| `institution_id` | `Integer`    | Institution identifier.                  |
| `subject_id`     | `Integer`    | Subject identifier.                      |
| `status`         | `String(1)`  | Subject processing status.               |
...
| `update_type`    | `String(50)` | Type of update performed.                |

The statuses used by the system include:
...
### `subject_indicator_status` table

The `subject_indicator_status` table allows the system to individually control the processing of each indicator for a subject and a specific actor.

| Column           | Type         | Description                                                         |
| ---------------- | ------------ | ------------------------------------------------------------------- |
| `institution_id` | `Integer`    | Institution identifier.                                             |
| `subject_id`     | `Integer`    | Subject identifier.                                                 |
| `actor`          | `String(20)` | Actor type associated with the indicator, such as student or tutor. |
| `indicator_name` | `String(50)` | Indicator name.                                                     |
| `status`         | `String(1)`  | Indicator processing status.                                        |
...
This table allows the system, for example, to identify whether a specific student or tutor indicator has already been processed for a particular subject.

### `local_indicators_students` table

The `local_indicators_students` table stores indicators calculated individually for each student within a subject.

| Column                                 | Type         | Description                                                   |
| -------------------------------------- | ------------ | ------------------------------------------------------------- |
| `institution_id`                       | `Integer`    | Institution identifier.                                       |
| `version`                              | `String(40)` | Moodle version.                                               |
| `subject_id`                           | `Integer`    | Subject identifier.                                           |
| `student_id`                           | `Integer`    | Student identifier.                                           |
| `n_posts_engagement`                   | `Integer`    | Number of posts used for the engagement indicator.            |
| `label_engagement`                     | `String(32)` | Engagement classification.                                    |
| `n_posts_motivation`                   | `Integer`    | Number of posts used for the motivation indicator.            |
| `label_motivation`                     | `String(32)` | Motivation classification.                                    |
| `grade_performance`                    | `Float`      | Grade used in the performance calculation.                    |
| `grade_comparative_performance`        | `Float`      | Comparative performance measure relative to the class.        |
| `label_performance`                    | `String(32)` | Performance classification.                                   |
| `mean_forum_interactions_cognitive`    | `Float`      | Mean cognitive interactions in forums.                        |
| `mean_quiz_interactions_cognitive`     | `Float`      | Mean cognitive interactions in quizzes.                       |
| `mean_assign_interactions_cognitive`   | `Float`      | Mean cognitive interactions in assignments.                   |
| `label_cognitive`                      | `String(32)` | Cognitive depth classification.                               |
| `n_responses_relation_teacher_student` | `Integer`    | Number of interactions between the student and teacher/tutor. |
| `label_relation_teacher_student`       | `String(32)` | Student-instructor interaction classification.                |
| `label_give_up`                        | `String(32)` | Classification related to dropout risk.                       |
...
This table represents the local/individual level, as each record corresponds to a student within a subject.

### `global_indicators_students` table

The `global_indicators_students` table stores student indicators aggregated at the subject level.
...
| `version`                                 | `String(40)` | Moodle version.                                        |
| `subject_id`                              | `Integer`    | Subject identifier.                                    |
| `mean_posts_engagement`                   | `Float`      | Mean engagement indicator among students.              |
| `label_engagement`                        | `String(32)` | Overall engagement classification.                     |
| `mean_posts_motivation`                   | `Float`      | Mean motivation indicator among students.              |
| `label_motivation`                        | `String(32)` | Overall motivation classification.                     |
| `mean_grade_performance`                  | `Float`      | Mean student performance.                              |
| `label_performance`                       | `String(32)` | Overall performance classification.                    |
| `mean_interactions_cognitive`             | `Float`      | Mean student cognitive depth.                          |
| `label_cognitive`                         | `String(32)` | Overall cognitive depth classification.                |
| `mean_responses_relation_teacher_student` | `Float`      | Mean student-instructor interaction.                   |
| `label_relation_teacher_student`          | `String(32)` | Overall student-instructor interaction classification. |
| `mean_give_up`                            | `Float`      | Mean/proportion related to dropout risk.               |
| `label_give_up`                           | `String(32)` | Overall dropout risk classification.                   |
...
This table represents the global subject level and is populated from the individual student results.

### `local_indicators_tutors` table

The `local_indicators_tutors` table stores indicators calculated individually for each tutor within a subject.
...
| `version`                            | `String(40)` | Moodle version.                                         |
| `subject_id`                         | `Integer`    | Subject identifier.                                     |
| `tutor_id`                           | `Integer`    | Tutor identifier.                                       |
| `median_forums_response_hours`       | `Float`      | Median forum response time, in hours.                   |
| `mean_forums_response_hours`         | `Float`      | Mean forum response time, in hours.                     |
| `total_response_forum`               | `Integer`    | Total number of forum responses.                        |
| `score_access`                       | `Float`      | Tutor access score.                                     |
| `mean_forums_response_hours_label`   | `String(32)` | Mean response time classification.                      |
| `median_forums_response_hours_label` | `String(32)` | Median response time classification.                    |
| `score_access_label`                 | `String(32)` | Access score classification.                            |
| `label_forums_response`              | `String(32)` | Overall forum response classification.                  |
| `num_response_fast_forum`            | `Integer`    | Number of fast responses.                               |
| `num_response_late_forum`            | `Integer`    | Number of late responses.                               |
| `num_response_normal_forum`          | `Integer`    | Number of normal responses.                             |
| `n_login`                            | `Integer`    | Number of logins performed by the tutor.                |
| `n_login_subject`                    | `Integer`    | Number of accesses to the subject.                      |
| `n_login_weekly`                     | `Integer`    | Weekly login frequency.                                 |
| `n_login_label`                      | `String(32)` | Login frequency classification.                         |
| `maximum_inactivity_days`            | `Integer`    | Maximum tutor inactivity period, in days.               |
| `n_login_weekly_label`               | `String(32)` | Weekly login frequency classification.                  |
| `label_access`                       | `String(32)` | Overall access classification.                          |
| `maximum_inactivity_days_label`      | `String(32)` | Maximum inactivity period classification.               |
| `n_corrections`                      | `Integer`    | Number of graded/corrected activities.                  |
| `n_corrections_with_feedback`        | `Integer`    | Number of activities graded with feedback.              |
| `percentage_feedback`                | `Float`      | Percentage of graded activities that received feedback. |
| `n_textual_feedback`                 | `Integer`    | Number of textual feedback entries.                     |
| `n_feedback_pdf`                     | `Integer`    | Number of feedback entries provided as files/PDFs.      |
| `n_corrections_label`                | `String(32)` | Number of corrections classification.                   |
| `n_corrections_with_feedback_label`  | `String(32)` | Classification of corrections with feedback.            |
| `percentage_feedback_label`          | `String(32)` | Feedback percentage classification.                     |
| `n_textual_feedback_label`           | `String(32)` | Textual feedback classification.                        |
| `n_feedback_pdf_label`               | `String(32)` | File feedback classification.                           |
| `label_feedback`                     | `String(32)` | Overall feedback classification.                        |
...
This table represents the local/individual tutor level.

### `global_indicators_tutors` table

The `global_indicators_tutors` table stores tutor indicators aggregated at the subject level.
...
| `version`               | `String(40)` | Moodle version.                       |
| `subject_id`            | `Integer`    | Subject identifier.                   |
| `score_global_forum`    | `Float`      | Global forum response score.          |
| `label_global_forum`    | `String(32)` | Global forum response classification. |
| `score_global_access`   | `Float`      | Global access score.                  |
| `label_global_access`   | `String(32)` | Global access classification.         |
| `score_global_feedback` | `Float`      | Global feedback score.                |
| `label_global_feedback` | `String(32)` | Global feedback classification.       |
...
The values are obtained by aggregating the individual indicators of the tutors associated with the subject.

### `scheduler_status` table

The `scheduler_status` table stores information about the execution of scheduled system processes.

| Column             | Type           | Description                                             |
| ------------------ | -------------- | ------------------------------------------------------- |
| `job_id`           | `String(100)`  | Unique identifier of the scheduled process.             |
| `channel`          | `String(50)`   | Channel associated with the process.                    |
| `process_id`       | `Integer`      | Identifier of the running process.                      |
| `next_run_at`      | `DateTime`     | Date and time scheduled for the next execution.         |
| `heartbeat_at`     | `DateTime`     | Date and time of the process's last activity signal.    |
| `last_started_at`  | `DateTime`     | Date and time when the last execution started.          |
| `last_finished_at` | `DateTime`     | Date and time when the last execution finished.         |
| `last_status`      | `String(20)`   | Status of the last execution.                           |
| `last_error`       | `String(1000)` | Error message from the last execution, when applicable. |
| `updated_at`       | `DateTime`     | Date and time of the last record update.                |
...
The records are primarily organized around the institution, Moodle version, and subject.

The indicator structure follows an aggregation relationship:
...
In addition to the indicator tables, the database maintains control tables responsible for monitoring processing:
...
The indicators are stored at two main levels:

| Level                 | Table                        | Unit of analysis         |
| --------------------- | ---------------------------- | ------------------------ |
| **Local — Student**   | `local_indicators_students`  | Student within a subject |
| **Global — Students** | `global_indicators_students` | Subject                  |
| **Local — Tutor**     | `local_indicators_tutors`    | Tutor within a subject   |
| **Global — Tutors**   | `global_indicators_tutors`   | Subject                  |

This separation allows Prisma to maintain both individual results, used for more detailed analyses, and aggregated results, used to present indicators at the subject level.

### Entity-relationship diagram

The conceptual structure of the Local Database can be represented by the following relationship:

<p align="center">
  <img src="docs/assets/DER_prisma.png" alt="Entity-relationship diagram" width="60%">
</p>

### Indicator persistence

The indicator persistence flow can be summarized as follows:
...
Thus, the Local Database acts as the persistence layer for the results produced by the Analysis System, keeping individual student and tutor data separate from aggregated indicators at the subject level.
